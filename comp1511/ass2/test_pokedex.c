...
// Sample data on MMM, the Pokemon with pokemon_id 354.
#define MMM_ID 354
#define MMM_NAME "MMM"
#define MMM_HEIGHT 40.7
#define MMM_WEIGHT 87.5
#define MMM_FIRST_TYPE FIRE_TYPE
#define MMM_SECOND_TYPE NONE_TYPE

// Sample data on GGG, the Pokemon with pokemon_id 1009.
#define GGG_ID 1009
#define GGG_NAME "GGG"
#define GGG_HEIGHT 40.7
#define GGG_WEIGHT 87.5
#define GGG_FIRST_TYPE FIRE_TYPE
#define GGG_SECOND_TYPE FIRE_TYPE
...
// creating the Pokemon Bulbasaur (using the helper
...
//to check if the program exit when a new pokemon with an id 
//that already exist in the pokedex added.
//also check if the first added pokemon is the current one.
static void another_test_add_pokemon(void) {
    printf("\n>> Testing add_pokemon\n");
    ...
    printf("    ... Checking if the Bulbasaur is the current one\n");
    assert(pokedex->head->selected == 1);
    
    printf("    ... Adding another Bulbasaur to the Pokedex\n");
    add_pokemon(pokedex, bulbasaur);  
    
    printf("    ... Destroying the Pokedex\n");
    destroy_pokedex(pokedex);

    printf("Failed. you should exit when second bulbasaur added\n");  
}

//to check if the function does exit when there is no pokemon yet 
//and if the current pokemon being "selected" after the function runs.
static void test_get_current_pokemon(void) {
    ...
    printf("    ... Adding Bulbasaur the Pokedex\n");
    add_pokemon(pokedex, bulbasaur);
    
    printf("    ... Running get_current_pokemon\n");
    get_current_pokemon(pokedex);
    
    printf("    ... Checking whether current pokemon is current\n");
    assert(pokedex->head->selected == 1);

    printf("    ... Destroying the Pokedex\n");
    destroy_pokedex(pokedex);

    printf(">> Passed first get_current_pokemon tests!\n");
    
    printf("    ... Creating another new Pokedex\n");
    Pokedex pokedex = new_pokedex();
    
    printf("you'll pass if program exit with a proper message\n");
...
//check if selected pokemon get found after
static void test_first_and_second_stage(void) {
    printf("\n>> Testing find_current_pokemon\n");
    
    //to see if function does nothing when there is no pokemon
    printf("    ... Running find_current_pokemon\n");

    printf("    ... Creating a new Pokedex\n");
    Pokedex pokedex = new_pokedex();

    printf("    ... Creating Bulbasaur, MMM ,GGG and Ivysaur\n");
    Pokemon bulbasaur = create_bulbasaur();
    Pokemon ivysaur = create_ivysaur();
    Pokemon MMM = create_MMM();
    Pokemon GGG = create_GGG();
    
    
    printf("    ... Adding Bulbasaur, MMM, GGG and Ivysaur to the Pokedex\n");
    add_pokemon(pokedex, bulbasaur);
    add_pokemon(pokedex, ivysaur);
    add_pokemon(pokedex, MMM);
    add_pokemon(pokedex, GGG);
    
    printf("    ... Running next_pokeon")
    next_pokemon(pokedex);
    
    printf("    ... Running next_pokeon")
    next_pokemon(pokedex);
    
    printf("checking next_pokemon");
    assert(is_same_pokemon(get_current_pokemon(pokedex), MMM));
    
    printf("    ... Running next_pokeon")
    next_pokemon(pokedex);
       
    //to see if function runs correctly when the current is the last
    printf("    ... Running next_pokeon")
    next_pokemon(pokedex);
    
    printf("checking next_pokemon");
    assert(is_same_pokemon(get_current_pokemon(pokedex), GGG));
    
    printf("   ... Running find_current_pokemon");
    find_current_pokemon(pokedex);

    printf("    ... Running prev_pokeon")
    ...
    printf(">> Passed first and second stage!\n");        
}
...
// Helper function to create MMM for testing purposes.
static Pokemon create_MMM(void) {
    Pokemon pokemon = new_pokemon(
            MMM_ID, MMM_NAME,
            MMM_HEIGHT, MMM_WEIGHT,
            MMM_FIRST_TYPE,
            MMM_SECOND_TYPE
    );
    return pokemon;
