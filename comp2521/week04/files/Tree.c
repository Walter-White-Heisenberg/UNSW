...
static Link insertSplay (Link t, Item it)
{
	...
	if (diff == 0) {
		...
		if (t->left == NULL) {
			t->left = newNode (it);
		} else if (t->within->ncompares++ && less (v, key (t->left->value))) {
		...
		if (t->right == NULL) {
			t->right = newNode (it);
		} else if (t->within->ncompares++ && less (key (t->right->value), v)) {
...
static Link insertAVL (Link t, Item it)
{
	...
	if ((dL - dR) > 1) {
		dL = depth(t->left->left);
		dR = depth(t->left->right);
		if ((dL - dR) > 0) {
			t = rotateR (t);
		} else {
			t->left = rotateL (t->left);
		...
		dL = depth(t->right->left);
		dR = depth(t->right->right);
		if ((dR - dL) > 0) {
			t = rotateL (t);
		} else {
			t->right = rotateR (t->right);
