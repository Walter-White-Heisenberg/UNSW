...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
	assert (g != NULL);
    int x = 0;
    for(VList special = g->connections[start]; special != NULL; special = special->next){
        if(special->v == end && ((idToType(start) != LAND || idToType(end) != LAND) || special->type != BOAT)){
            type[x] = special->type;
            x++;
        }
        if(special->type == BOAT){
            for(VList S = g->connections[end]; S != NULL; S = S->next){
                if(idToType(start) == LAND && idToType(end) == LAND && special->v == S->v ){
                    type[x] = BOAT;
                    x++;
                }
            }
	...
	return x;  // to keep the compiler happy
}
